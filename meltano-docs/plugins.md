...
```bash
meltano config set <extractor> _catalog <path>

meltano el <extractor> <loader> --catalog <path>

# For example:
meltano config set tap-gitlab _catalog extract/tap-gitlab.catalog.json
...
```bash
meltano config set <extractor> _load_schema <schema>

# For example:
meltano config set tap-gitlab _load_schema gitlab_data
...
```bash
meltano config set <extractor> _metadata <stream> <key> <value>
meltano config set <extractor> _metadata <stream> <property> <key> <value>

# For example:
meltano config set tap-postgres _metadata some_stream_id replication-method INCREMENTAL
meltano config set tap-postgres _metadata some_stream_id replication-key created_at
meltano config set tap-postgres _metadata some_stream_id created_at is-replication-key true
...
```bash
meltano config set <extractor> _schema <entity> <attribute> <schema description>
meltano config set <extractor> _schema <entity> <attribute> <key> <value>

# For example:
meltano config set tap-postgres _metadata some_stream_id created_at type '["string", "null"]'
meltano config set tap-postgres _metadata some_stream_id created_at format date-time
```
...
```bash
meltano config set <extractor> _select '["<entity>.<attribute>", ...]'

meltano select <extractor> <entity> <attribute>

# For example:
# Stream-only patterns (both lines are equivalent)
meltano config set tap-gitlab _select '["project_members", "commits"]'
meltano config set tap-gitlab _select '["project_members.*", "commits.*"]'

# Mix of stream-only and specific properties
meltano config set tap-gitlab _select '["project_members", "commits.id", "commits.author_name"]'
...
```bash
meltano config set tap-smoke-test _select '["animals\\.data.id","animals\\.data.verified"]'
...
```bash
meltano config set <extractor> _select_filter '["<entity>", ...]'
meltano config set <extractor> _select_filter '["!<entity>", ...]'
...
meltano config set tap-gitlab _select_filter '["commits", "issues"]'

# Exclude specific streams
meltano config set tap-gitlab _select_filter '["!project_members", "!logs"]'
...
```bash
meltano config set <extractor> _state <path>

meltano el <extractor> <loader> --state <path>

# For example:
meltano config set tap-gitlab _state extract/tap-gitlab.state.json
...
```bash
meltano config set <extractor> _use_cached_catalog false

# For example:
meltano config set tap-gitlab _use_cached_catalog false
```
...
```bash
meltano config set <loader> _dialect <dialect>

# For example:
meltano config set target-example-db _dialect example-db
```
...
```bash
meltano config set <transform> _package_name <name>

# For example:
meltano config set dbt-facebook-ads _package_name facebook_ads
```
...
```bash
{% raw %}
meltano config set <transform> _vars <key> <value>

# For example
meltano config set --plugin-type=transform tap-gitlab _vars schema "{{ env_var('DBT_SOURCE_SCHEMA') }}"
{% endraw %}
...
```bash
meltano config set <bundle> _update <path> <true/false>

# For example:
meltano config set --plugin-type=files dbt _update transform/dbt_project.yml false
meltano config set --plugin-type=files dbt _update profiles/*.yml true
```
